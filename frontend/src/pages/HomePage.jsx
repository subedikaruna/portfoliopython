...
export default function HomePage({ profile }) {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_BASE_URL}/api/projects/`)
      .then((res) => res.json())
      .then((data) => {
        // Filter projects where featured is true
        const featured = data.filter(
          (project) =>
            project.featured === true || project.featured === 'true'
        );

        // Fallback to showing the first 3 projects
        setFeaturedProjects(
          featured.length > 0 ? featured : data.slice(0, 3)
        );
  ...
  return (
    <div className="space-y-16 py-6">
      ...
      <section className="flex flex-col md:flex-row items-center justify-between gap-8 py-10">
        <div className="flex-1 space-y-4">
          ...
          <p className="text-base opacity-80 leading-relaxed max-w-2xl">
            {profile?.bio ||
              'Building modern, interactive full-stack web applications with React, Django, and AI integrations.'}
          </p>
      ...
      <section className="space-y-6">
        ...
        {loading ? (
          <div className="py-12 text-center opacity-70 animate-pulse">
          ...
          <p className="opacity-70 text-sm">
            No projects found. Add projects in Django Admin.
          </p>
        )}
      </section>
    </div>
